...
By default the CLI connects to the hosted Corvin service (`wss://api.corvin.dev/v2/ws`). To use your own deployed server, set these in `~/.corvin/config`:
...
|---|---|---|
| `WEB_SOCKET_URL` | `wss://api.corvin.dev/v2/ws` | WebSocket URL of the Corvin server |
| `API_BASE_URL` | `https://api.corvin.dev/v2/api` | REST API base URL of the Corvin server |
